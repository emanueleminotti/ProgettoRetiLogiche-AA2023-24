...
\begin{itemize}
  \item \textbf{{first\_data\_is\_zero}:} flag utilizzato per la gestione del caso particolare in cui il primo valore sia zero.
  \item \textbf{{curr\_addr}:} memorizza l'indirizzo corrente al quale andremo a leggere/scrivere i dati in memoria.
  ...
  \item \textbf{{en\_word\_count}:} abilita il modulo che fa da contatore di parole.
  ...
  \item \textbf{{new\_data}:} indica che un nuovo valore valido è stato letto dalla memoria.
  \item \textbf{{done\_processing}:} indica il termine della computazione.
\end{itemize}
\subsection{Moduli}
%\begin{figure}[!h]
%  \includegraphics[width = 15cm]{images/schema_a_blocchi_componente.pdf}
%  \caption{Schema a blocchi del componente}
%\end{figure}
% schema a blocchi componente
% presentazione dei process
\subsubsection{Address counter}
 Questo modulo calcola l'indirizzo della cella di memoria del dato da leggere/scrivere. Ad inizio computazione curr\_addr viene posto uguale a i\_add, successivamente viene incrementato di un'unità sul fronte di salita del clock se il segnale di en\_addr\_count è stato alzato. In presenza di un segnale di reset curr\_addr viene posto a 0.
\subsubsection{Word counter}
 Questo modulo conta fino al numero di parole K. Il conteggio parziale, inizialmente posto a 0, viene incrementato di un'unità ogni volta che viene letta una parola dalla memoria. Una volta raggiunto il valore K dato, il segnale di done\_processing viene portato a 1 in modo da portare a conclusione la computazione. In presenza di un segnale di reset sia word\_count che done\_processing vengono riportati a 0.
\subsubsection{Credibility counter}
 Questo modulo calcola il valore di credibilità che verrà scritto man mano nella memoria durante la computazione. Partendo dal valore 31 di default, esso rimane tale se viene letta una parola diversa da zero, mentre viene decrementato di un'unità (fino ad arrivare al massimo a 0) per ogni zero consecutivo che viene letto. In presenza di un segnale di reset, credibility torna al suo valore di default.
