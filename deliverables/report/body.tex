...
\subsubsection{Data register}
 Aggiorna tutti i segnali associati alla lettura del dato corrente, in particolare ad ogni ciclo di clock (se non c'è stata reinizializzazione) $current\_data$ viene posto uguale a $i\_mem\_data$, e se $new\_data$ è uguale a 1 allora $first\_data\_is\_zero$ viene mandato a zero e $last\_valid\_data$ viene anch'esso posto uguale a $i\_mem\_data$.  
...
\subsubsection{Macchina a stati}
  \par L'ultimo modulo del componente progettato è la macchina a stati finiti, che ha il compito di gestire l'intero processo di elaborazione della sequenza in ingresso. La FSM coordina le operazioni degli altri moduli, assicurando che ogni fase dell'elaborazione avvenga nell'ordine corretto e nel momento opportuno.
  ...
  \par \begin{itemize}
    \item S0: rappresenta lo stato iniziale della FSM, viene eseguita la transizione verso S1 una volta che $i\_rst$ viene abbassato. 
    \item S1: è uno stato di setup in cui avviene una reinizializzazione di tutti i moduli, in modo da poter cominciare una nuova computazione non appena il segnale $i\_start$ viene alzato.
    \item S2: stato di attesa dell'arrivo di una nuova parola in un ciclo che scorre l'intera stringa. Una volta arrivati al termine, dopo che viene alzato il segnale di $done\_processing$ avviene una transizione verso lo stato S7. 
    \item S4: in questo stato avviene la gestione dei vari segnali in base al valore letto dalla memoria. Se il valore ricevuto è il primo della stringa ed è uno zero, il segnale di selezione viene cambiato in modo da portare all'uscita un valore nullo, se invece si tratta di uno zero in mezzo alla sequenza viene alzato il segnale per attivare il Credibility Counter e la selezione viene modificata in modo da portare all'uscita l'ultimo valore valido letto (contenuto in $last\_valid\_data$). Se invece il valore letto è diverso da zero viene reinizializzato il Credibility counter, viene alzato il segnale di $new\_data$ e la selezione modificata in modo da portare all'uscita il dato corrente. Infine, indipendetemente dal valore letto, viene abilitata la scrittura in memoria e attivato l'Address counter.
    \item S6: stato che serve a modificare la selezione per la scrittura della credibilità (verrà scritto 0 se ancora non è mai stato letto un valore non nullo, altrimenti verrà scritto il valore contenuto in $Credibility$). Vengono anche alzati i segnali per l'abilitazione della scrittura e dei moduli Address counter e Word counter.
    \item S7: ulteriore stato di attesa che aspetta che il segnale $i\_start$ venga abbassato in modo da poter tornare allo stato di setup S1.
  \end{itemize}
  \begin{figure}[H]
